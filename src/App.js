...
function getTimeBg() {
  ...
  let a = TIME_STOPS[0], b = TIME_STOPS[1];
  for (let i = 0; i < TIME_STOPS.length - 1; i++) {
    if (total >= TIME_STOPS[i].h * 60 && total < TIME_STOPS[i+1].h * 60) {
      a = TIME_STOPS[i]; b = TIME_STOPS[i+1]; break;
    }
  }
  const t = (total - a.h * 60) / ((b.h - a.h) * 60);
...
function weekChecksCount(history, skillId, todayChecks) {
  const ws = getWeekStart(), tds = todayStr();
  const fromHist = history.filter(h => { const hd = new Date(h.date+'T00:00:00'); return hd >= ws && h.date !== tds && h.checks.includes(skillId); }).length;
...
function checkNewAchievements({ skills, xpMap, history, todayState, unlockedIds }) {
  ...
  if (todayState.checks.length > 0 || todayState.diary) allH.push({ ...todayState });
...
const GLOBAL_CSS = `
  *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
  ...
  body { background:var(--bg, #080808); color:#e0d5c0; font-family:'Segoe UI',system-ui,-apple-system,sans-serif; min-height:100vh; overflow-x:hidden; transition:background 2min ease; }
  ...
  return (
    <header style={{position:'fixed',top:0,left:0,right:0,zIndex:100,background:'rgba(8,8,8,0.93)',backdropFilter:'blur(14px)',borderBottom:'1px solid rgba(201,168,76,0.12)',padding:'11px 20px'}}>
      <div style={{maxWidth:700,margin:'0 auto',display:'flex',alignItems:'center',gap:14}}>
        ...
        {streak > 0 && <div style={{flexShrink:0,display:'flex',alignItems:'center',gap:4,fontSize:13,color:'#c9a84c',fontWeight:600}}>🔥 {streak}</div>}
      </div>
    </header>
  );
}
...
function MiniCalendar({ history, selectedDate, onSelect }) {
  ...
  const historyDates = new Set(history.filter(h => h.checks.length > 0).map(h => h.date));
  const moodByDate   = {};
  history.forEach(h => { if (h.mood) moodByDate[h.date] = h.mood; });
  ...
  const monthName = new Date(viewYear, viewMonth, 1).toLocaleString('pt-BR', { month:'long' });
  const canGoNext = !(viewYear === now.getFullYear() && viewMonth === now.getMonth());

  return (
    <div style={{background:'rgba(255,255,255,0.025)',borderRadius:12,padding:'14px 16px',marginBottom:18,border:'1px solid rgba(255,255,255,0.055)'}}>
      {/* Month nav */}
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:12}}>
        <button onClick={prevMonth} style={{background:'transparent',border:'none',color:'#666',fontSize:16,padding:'2px 8px'}}>‹</button>
        <span style={{fontSize:13,color:'#999',fontWeight:500,textTransform:'capitalize'}}>{monthName} {viewYear}</span>
        <button onClick={nextMonth} disabled={!canGoNext} style={{background:'transparent',border:'none',color:canGoNext?'#666':'#2a2a2a',fontSize:16,padding:'2px 8px'}}>›</button>
      </div>
      {/* Weekday labels */}
      <div style={{display:'grid',gridTemplateColumns:'repeat(7,1fr)',gap:3,marginBottom:5}}>
        {WEEK_DAYS.map(d => <div key={d} style={{textAlign:'center',fontSize:9,color:'#444',fontWeight:500}}>{d}</div>)}
      </div>
      {/* Days grid */}
      <div style={{display:'grid',gridTemplateColumns:'repeat(7,1fr)',gap:3}}>
        ...
        {Array.from({length: daysInMonth}, (_, i) => {
          ...
          return (
              ...
              style={{
                ...
                background: isSelected
                  ...
                  : isToday
                    ...
                    : hasEntry ? 'rgba(255,255,255,0.04)' : 'transparent',
                border: isSelected
            ...
            >
              {mood ? (
                <span style={{fontSize:13,lineHeight:1}}>{moodEmoji[mood]}</span>
              ) : (
                <span style={{fontSize:11,color: isSelected?'#c9a84c':isToday?'#c9a84c':'#666',fontWeight:isToday||isSelected?600:400}}>{day}</span>
              )}
              {hasEntry && !mood && <div style={{width:3,height:3,borderRadius:'50%',background: isSelected?'#c9a84c':'rgba(201,168,76,0.5)'}} />}
            </div>
          );
        })}
      </div>
...
// ── Past day view ─────────────────────────────────────────────────────────────
function PastDayView({ date, entry, skills, isUnlocked, onToggleLock, onUpdateEntry }) {
  const [diary, setDiary]   = useState(entry?.diary || '');
  const [mood,  setMood]    = useState(entry?.mood  || null);
  const [saved, setSaved]   = useState(false);

  useEffect(() => { setDiary(entry?.diary || ''); setMood(entry?.mood || null); }, [date]);

  const save = () => {
    onUpdateEntry(date, { ...entry, diary, mood });
    setSaved(true);
    setTimeout(() => setSaved(false), 1500);
  };

  const checks  = entry?.checks || [];
  const done    = skills.filter(s => checks.includes(s.id));
  const total   = skills.length;
  const pct     = total > 0 ? Math.round((checks.length / total) * 100) : 0;
  ...
  return (
    <div className="slide-down">
      ...
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:16}}>
        <div>
          <div className="golden" style={{fontSize:18,fontWeight:700}}>{dateLabel(date)}</div>
          <div style={{fontSize:11,color:'#555',marginTop:2}}>
            {checks.length} atividade{checks.length !== 1 ? 's' : ''} · {pct}% · {checks.length * XP_PER} XP
          </div>
        </div>
        <button
          onClick={onToggleLock}
          title={isUnlocked ? 'Travar (somente leitura)' : 'Destrancar para editar'}
          style={{padding:'8px 14px',borderRadius:8,fontSize:14,background:isUnlocked?'rgba(201,168,76,0.12)':'rgba(255,255,255,0.04)',border:isUnlocked?'1px solid rgba(201,168,76,0.35)':'1px solid rgba(255,255,255,0.1)',color:isUnlocked?'#c9a84c':'#555',transition:'all .25s ease'}}
        >
          {isUnlocked ? '🔓 Editável' : '🔒 Travado'}
        </button>
      </div>

      {/* Mood (read-only or editable) */}
      <div style={{background:'rgba(255,255,255,0.025)',borderRadius:12,padding:'14px 16px',marginBottom:14,border:'1px solid rgba(255,255,255,0.055)'}}>
        <div style={{fontSize:11,color:'#555',letterSpacing:.7,marginBottom:10}}>HUMOR</div>
        <div style={{display:'flex',gap:6}}>
          {MOODS.map(m => {
            const active = mood === m.value;
            return (
              <button key={m.value} onClick={() => isUnlocked && setMood(active ? null : m.value)}
                style={{flex:1,padding:'8px 4px',borderRadius:8,display:'flex',flexDirection:'column',alignItems:'center',gap:4,background:active?'rgba(201,168,76,0.1)':'rgba(255,255,255,0.02)',border:active?'1px solid rgba(201,168,76,0.4)':'1px solid rgba(255,255,255,0.04)',cursor:isUnlocked?'pointer':'default',opacity:!isUnlocked&&!active?.4:1,transition:'all .2s'}}>
                <span style={{fontSize:18}}>{m.emoji}</span>
                <span style={{fontSize:9,color:active?'#c9a84c':'#444'}}>{m.label}</span>
              </button>
            );
          })}
        </div>
      ...
      {/* Activities (always read-only) */}
      <div style={{background:'rgba(255,255,255,0.025)',borderRadius:12,padding:'14px 16px',marginBottom:14,border:'1px solid rgba(255,255,255,0.055)'}}>
        <div style={{fontSize:11,color:'#555',letterSpacing:.7,marginBottom:10}}>ATIVIDADES</div>
        {done.length === 0 ? (
          <div style={{color:'#383838',fontSize:13}}>Nenhuma atividade registrada.</div>
        ) : (
          <div style={{display:'flex',flexWrap:'wrap',gap:8}}>
            {done.map(s => (
              <div key={s.id} style={{display:'flex',alignItems:'center',gap:7,padding:'6px 12px',borderRadius:8,background:'rgba(201,168,76,0.07)',border:'1px solid rgba(201,168,76,0.2)'}}>
                <span>{s.emoji}</span>
                <span style={{fontSize:12,color:'#c8c0b0'}}>{s.name}</span>
              </div>
            ))}
          </div>
        )}
      ...
      {/* Diary */}
      <div style={{background:'rgba(255,255,255,0.025)',borderRadius:12,padding:'14px 16px',marginBottom: isUnlocked ? 14 : 0,border:'1px solid rgba(255,255,255,0.055)'}}>
        <div style={{fontSize:11,color:'#555',letterSpacing:.7,marginBottom:10}}>DIÁRIO</div>
        {isUnlocked ? (
          <textarea value={diary} onChange={e => setDiary(e.target.value)} placeholder="Escreva sobre esse dia..." style={{width:'100%',minHeight:90,background:'transparent',border:'none',color:'#b8b0a0',fontSize:14,lineHeight:1.65,resize:'vertical',fontFamily:'inherit'}} />
        ) : (
          <div style={{fontSize:13,color:'#666',fontStyle:'italic',lineHeight:1.6,minHeight:40}}>
            {entry?.diary || <span style={{color:'#383838'}}>Sem entrada de diário.</span>}
          </div>
        )}
      </div>
      ...
      {/* Save button */}
      {isUnlocked && (
        <button onClick={save} style={{width:'100%',padding:'11px',borderRadius:9,fontSize:13,fontWeight:500,background:saved?'rgba(92,184,92,0.15)':'rgba(201,168,76,0.12)',border:saved?'1px solid rgba(92,184,92,0.4)':'1px solid rgba(201,168,76,0.35)',color:saved?'#5cb85c':'#c9a84c',transition:'all .25s ease'}}>
...
function ActivityCard({ skill, xp, checked, onToggle, onDelete, weekCount, weekGoal }) {
  ...
  const handleClick = (e) => {
    ...
    ref.current.appendChild(rip); setTimeout(() => rip.remove(), 600);
    ...
    <div ref={ref} onClick={handleClick} className="card-item" style={{position:'relative',overflow:'hidden',display:'flex',alignItems:'center',gap:14,padding:'13px 16px',borderRadius:10,cursor:onDelete?'default':'pointer',background:checked?'rgba(201,168,76,0.07)':'rgba(255,255,255,0.022)',border:`1px solid ${checked?'rgba(201,168,76,0.38)':'rgba(255,255,255,0.055)'}`,boxShadow:checked?'0 0 22px rgba(201,168,76,0.08)':'none'}}>
      ...
      <div style={{flex:1,minWidth:0}}>
        ...
        <div style={{display:'flex',alignItems:'center',gap:8,marginTop:3}}>
          ...
          {weekGoal > 0 && <span style={{fontSize:10,color:goalMet?'#c9a84c':'#555'}}>{goalMet?'✓ ':''}{weekCount}/{weekGoal}× semana</span>}
        </div>
        {weekGoal > 0 && (
...
// ── Today tab ─────────────────────────────────────────────────────────────────
function TodayTab({ skills, checks, xpMap, onToggle, diary, onDiaryChange, onAddActivity, onDeleteSkill, mood, onMoodChange, goals, history, selectedDate, onSelectDate, unlockedDates, onToggleLock, onUpdateEntry }) {
  ...
  const pastEntry     = history.find(h => h.date === selectedDate);
  const isUnlocked    = unlockedDates.includes(selectedDate);

  return (
    <div className="tab-panel">
      <MiniCalendar history={history} selectedDate={selectedDate} onSelect={onSelectDate} />

      {/* Back to today button */}
      {isPast && (
        <button onClick={() => onSelectDate(today)} style={{width:'100%',padding:'9px',marginBottom:14,background:'transparent',border:'1px solid rgba(255,255,255,0.08)',borderRadius:9,color:'#666',fontSize:13,transition:'all .2s'}} onMouseOver={e=>e.currentTarget.style.color='#c9a84c'} onMouseOut={e=>e.currentTarget.style.color='#666'}>
          ← Voltar para hoje
        </button>
      )}
      ...
      {isPast && (
        <PastDayView
          ...
          onToggleLock={() => onToggleLock(selectedDate)}
      ...
      {isToday && (
        <>
          ...
          <div style={{background:'rgba(255,255,255,0.025)',borderRadius:12,padding:'16px 18px',marginBottom:18,border:'1px solid rgba(255,255,255,0.055)'}}>
            ...
            {done===total&&total>0 && <div className="shimmer" style={{marginTop:12,fontSize:14,fontWeight:700,textAlign:'center',animation:'perfectDay .5s ease forwards, shimmer 2.5s linear infinite'}}>✦ Dia perfeito! ✦</div>}
          </div>

          <div style={{display:'flex',flexDirection:'column',gap:7,marginBottom:12}}>
            {skills.map(s => (
              <ActivityCard key={s.id} skill={s} xp={xpMap[s.id]||0} checked={checks.includes(s.id)}
                onToggle={editMode?()=>{}:onToggle} onDelete={editMode?onDeleteSkill:null}
                weekCount={weekChecksCount(history, s.id, checks)} weekGoal={goals[s.id]||0}
              />
            ))}
          </div>
...
function ImportModal({ onClose }) {
  ...
  const apply = () => { if (!code.trim()) return; applyImportCode(code) ? window.location.reload() : setError('Código inválido.'); };
  return (
    <div onClick={onClose} style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.82)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:200,padding:20}}>
      <div onClick={e=>e.stopPropagation()} style={{background:'#0d0d0d',border:'1px solid rgba(201,168,76,0.22)',borderRadius:14,padding:'26px 22px',width:'100%',maxWidth:420,animation:'levelUpIn .28s ease'}}>
        <div className="golden" style={{fontSize:17,fontWeight:600,marginBottom:8}}>Importar dados</div>
        <textarea value={code} onChange={e=>{setCode(e.target.value);setError('');}} placeholder="Cole o código aqui..." style={{width:'100%',height:100,background:'rgba(255,255,255,0.04)',border:`1px solid ${error?'rgba(192,80,80,0.4)':'rgba(255,255,255,0.08)'}`,borderRadius:8,color:'#ccc',fontSize:11,padding:'10px',resize:'none',fontFamily:'monospace'}} />
        {error && <div style={{color:'#c05050',fontSize:12,marginTop:6}}>{error}</div>}
        <div style={{display:'flex',gap:9,marginTop:12}}>
...
function PanelTab({ history, skills, xpMap, onDeleteSkill, onAddActivity, goals, onGoalChange, reminder, onReminderChange, achievements }) {
  const [showExport,setShowExport]=useState(false), [showImport,setShowImport]=useState(false);
  ...
  const requestReminder = (enabled, hour) => {
    ...
    } else onReminderChange({enabled,hour});
  };
    ...
    <div className="tab-panel">
      ...
      <div style={{marginBottom:28}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(5,1fr)',gap:8}}>
          {ACHIEVEMENTS.map(a => {
            ...
            return (
              <div key={a.id} title={`${a.name} — ${a.desc}${info?`\n${info.unlockedAt}`:''}`} style={{display:'flex',flexDirection:'column',alignItems:'center',gap:5,padding:'10px 6px',borderRadius:10,background:unlocked?'rgba(201,168,76,0.09)':'rgba(255,255,255,0.02)',border:`1px solid ${unlocked?'rgba(201,168,76,0.3)':'rgba(255,255,255,0.04)'}`,opacity:unlocked?1:.4,transition:'all .3s ease'}}>
              ...
              return (
                <div key={i} style={{padding:'12px 15px',background:'rgba(255,255,255,0.022)',borderRadius:10,border:'1px solid rgba(255,255,255,0.048)'}}>
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:6}}>
                    <span style={{fontSize:12,color:'#666'}}>{entry.date}{entry.mood&&<span style={{marginLeft:8}}>{MOODS.find(m=>m.value===entry.mood)?.emoji}</span>}</span>
                    <span style={{fontSize:12,color:'#c9a84c'}}>+{entry.checks.length*XP_PER} XP · {pct}%</span>
                  </div>
                  <div style={{fontSize:18,letterSpacing:2,marginBottom:entry.diary?6:0}}>{done.map(s=>s.emoji).join(' ')||<span style={{color:'#333',fontSize:13}}>—</span>}</div>
      ...
      <div style={{marginBottom:28,padding:'16px',background:'rgba(255,255,255,0.022)',borderRadius:12,border:'1px solid rgba(255,255,255,0.05)'}}>
        ...
        <div style={{display:'flex',alignItems:'center',gap:12,marginBottom:10}}>
          <button onClick={()=>requestReminder(!reminder.enabled,reminder.hour)} style={{width:44,height:24,borderRadius:12,background:reminder.enabled?'rgba(201,168,76,0.3)':'rgba(255,255,255,0.06)',border:`1px solid ${reminder.enabled?'rgba(201,168,76,0.5)':'rgba(255,255,255,0.1)'}`,position:'relative',transition:'all .3s ease',flexShrink:0}}>
          ...
          <span style={{fontSize:13,color:reminder.enabled?'#bbb':'#555'}}>{reminder.enabled?'Ativado':'Desativado'}</span>
        </div>
        {reminder.enabled && (
          <div style={{display:'flex',alignItems:'center',gap:10}}>
            <span style={{fontSize:13,color:'#777'}}>Lembrar às</span>
            <select value={reminder.hour} onChange={e=>onReminderChange({...reminder,hour:Number(e.target.value)})} style={{background:'rgba(255,255,255,0.06)',border:'1px solid rgba(255,255,255,0.1)',borderRadius:6,color:'#c9a84c',padding:'5px 10px',fontSize:13}}>
...
function MoodChart({ history }) {
  ...
  const moodByDay={};
  history.filter(h=>h.date.startsWith(mk)).forEach(h=>{if(h.mood)moodByDay[parseInt(h.date.split('-')[2],10)]=h.mood;});
  if (Object.keys(moodByDay).length===0) return null;
  const MC={1:'#c05050',2:'#c07a30',3:'#888',4:'#6aaa6a',5:'#c9a84c'};
  return (
    <div style={{marginBottom:22}}>
      <div style={{fontSize:11,color:'#555',letterSpacing:.7,marginBottom:14}}>HUMOR DO MÊS</div>
      <div style={{display:'flex',flexWrap:'wrap',gap:5}}>
        {Array.from({length:daysInMonth},(_,i)=>{
          const day=i+1, mood=moodByDay[day], isToday=day===d.getDate();
          return (
            <div key={day} title={mood?MOODS.find(m=>m.value===mood)?.label:'Sem registro'} style={{width:30,height:30,borderRadius:6,display:'flex',alignItems:'center',justifyContent:'center',fontSize:14,background:mood?`${MC[mood]}22`:'rgba(255,255,255,0.025)',border:`1px solid ${isToday?'rgba(201,168,76,0.4)':mood?`${MC[mood]}55`:'rgba(255,255,255,0.04)'}`,transition:'all .2s'}}>
...
function SummaryTab({ history, skills }) {
  const [aiText,setAiText]=useState(''), [loading,setLoading]=useState(false), [aiError,setAiError]=useState('');
  const d=new Date(), mk=`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}`;
  const monthH=history.filter(h=>(h.monthKey||h.date.substring(0,7))===mk);
  const activeDays=monthH.length, totalXp=monthH.reduce((a,h)=>a+h.checks.length*XP_PER,0);
  ...
  const generate=async()=>{
    ...
    try {
      ...
      const res=await fetch('https://api.anthropic.com/v1/messages',{method:'POST',headers:{'Content-Type':'application/json','x-api-key':process.env.REACT_APP_ANTHROPIC_API_KEY||'','anthropic-version':'2023-06-01','anthropic-dangerous-direct-browser-access':'true'},body:JSON.stringify({model:'claude-sonnet-4-20250514',max_tokens:1000,messages:[{role:'user',content:`Você é um coach pessoal empático. Com base nas entradas de diário abaixo, escreva um panorama em português de no máximo 4 frases, empático e direto, sem citar datas específicas.\n\n${entries}`}]})});
      const json=await res.json();
      if(json.content?.[0]?.text) setAiText(json.content[0].text);
      else setAiError(json.error?.message||'Resposta inesperada.');
...
function AddModal({ onAdd, onClose }) {
  const [emoji,setEmoji]=useState('⭐'), [name,setName]=useState('');
...
export default function App() {
  const [skills,       setSkills]       = useState(()=>{ try{return JSON.parse(localStorage.getItem(SK.skills))||DEFAULT_SKILLS;}catch{return DEFAULT_SKILLS;} });
  const [xpMap,        setXpMap]        = useState(()=>{ try{return JSON.parse(localStorage.getItem(SK.xp))||{};}catch{return {};} });
  const [history,      setHistory]      = useState(()=>{ try{return(JSON.parse(localStorage.getItem(SK.history))||[]).map(h=>({...h,monthKey:h.monthKey||h.date.substring(0,7)}));}catch{return[];} });
  const [todayState,   setTodayState]   = useState(()=>{ const t=todayStr(); try{const s=JSON.parse(localStorage.getItem(SK.today));if(s?.date===t)return s;}catch{} return{date:t,checks:[],diary:'',mood:null}; });
  const [achievements, setAchievements] = useState(()=>{ try{return JSON.parse(localStorage.getItem(SK.achievements))||[];}catch{return[];} });
  const [goals,        setGoals]        = useState(()=>{ try{return JSON.parse(localStorage.getItem(SK.goals))||{};}catch{return {};} });
  const [reminder,     setReminder]     = useState(()=>{ try{return JSON.parse(localStorage.getItem(SK.reminder))||{enabled:false,hour:21};}catch{return{enabled:false,hour:21};} });
  const [unlockedDates,setUnlockedDates]= useState(()=>{ try{return JSON.parse(localStorage.getItem(SK.unlocked))||[];}catch{return[];} });
  ...
  const [bgColor,      setBgColor]      = useState(getTimeBg());

  // Update bg every minute
  useEffect(() => {
  ...
  useEffect(()=>{ localStorage.setItem(SK.unlocked,     JSON.stringify(unlockedDates));},[unlockedDates]);
  ...
  useEffect(()=>{
    ...
    if(now.getHours()>=reminder.hour&&todayState.checks.length===0){
      ...
      if(Notification.permission==='granted') new Notification('Life Tracker 🎯',{body:'Não esqueça de registrar seu dia!'});
    }
  },[]);
  ...
  const handleToggle = useCallback((skillId)=>{
    ...
    setXpMap(prev=>{
      const next={...prev}, oldXp=next[skillId]||0;
      next[skillId]=Math.max(0,oldXp+(wasChecked?-XP_PER:XP_PER));
      if(!wasChecked){const oldLvl=calcLevel(oldXp).lvl,newLvl=calcLevel(next[skillId]).lvl;if(newLvl>oldLvl){const sk=skills.find(s=>s.id===skillId);if(sk)setTimeout(()=>setLevelUpSkill(sk),320);}}
      return next;
    });
  ...
  const handleToggleLock   = useCallback((date)=>{
    setUnlockedDates(prev=>prev.includes(date)?prev.filter(d=>d!==date):[...prev,date]);
  },[]);
  const handleUpdateEntry  = useCallback((date,updatedEntry)=>{
    setHistory(prev=>{
      const idx=prev.findIndex(h=>h.date===date);
      if(idx<0) return [...prev,{...updatedEntry,monthKey:monthKey(date)}];
      const n=[...prev]; n[idx]={...updatedEntry,monthKey:monthKey(date)}; return n;
    });
  },[]);
  ...
  return (
    <>
      ...
      <div style={{position:'relative',zIndex:2,maxWidth:700,margin:'0 auto',padding:'80px 15px 32px'}}>
        ...
        {tab==='today' && (
          <TodayTab
            skills={skills} checks={todayState.checks} xpMap={xpMap} onToggle={handleToggle}
            ...
            unlockedDates={unlockedDates} onToggleLock={handleToggleLock}
            onUpdateEntry={handleUpdateEntry}
          />
        )}
        {tab==='panel'   && <PanelTab history={history} skills={skills} xpMap={xpMap} onDeleteSkill={handleDeleteSkill} onAddActivity={()=>setShowAddModal(true)} goals={goals} onGoalChange={handleGoalChange} reminder={reminder} onReminderChange={setReminder} achievements={achievements} />}
      ...
      {currentAch   && <AchievementModal id={currentAch} onClose={()=>setAchQueue(prev=>prev.slice(1))} />}
